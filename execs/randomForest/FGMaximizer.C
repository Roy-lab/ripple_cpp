...
FGMaximizer::FGMaximizer()
{
	...
	//mode=Regression; // default: regression mode
	regressionMode=true;
...
// set printing mode -- default is 0=false
int 
FGMaximizer::setPrintAllTrees(bool printTrees)
{
	printAllTrees=printTrees;
	return 0;
}
...
/*
* Sets the mode in which to run the FGMaximizer: regression or classification
* DC added
*/
int 
...
FGMaximizer::findBestGraphs_PriorGraph(double lambda,const char* projectFName)
{
	...
	for(int i=0;i<currFg->getFactorCnt();i++)
	{
		...
		for(int t=0;t<rtreeSet.size();t++)
		{
			...
			// Print something for each tree, if requested
			if (printAllTrees)
			{
				...
				aPotFunc->dumpExpressionTab(rtree, sFactor->fId, expFile);
		...
		double err;
		// Training error (print to screen)
		if (regressionMode)
		{
			err=aPotFunc->getMSE();
		}
		...
		{
			// pass unopened ofstream
			//ofstream nostream;
			//cout << "GETTING TRAINING CLASSIFICATION " << endl;
			//err=aPotFunc->getTestClassError(trainEvMgr, nostream);
			//cout << "----" << endl;
		}
