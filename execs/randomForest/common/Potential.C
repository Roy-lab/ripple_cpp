...
/*
* Sets whether to do regression (true) or classification (false).
*/
int 
...
Potential::populateMe(double regval,int treeCnt)
{
	...
	for(int t=0;t<treeCnt;t++)
	{
		...
		// set regression/classification to match the Potential's mode
		dtree->setRegressionMode(regressionMode);
		...
		int randvarCnt;
		if (regressionMode) // regression mode -- nvariables/3
		{
			randvarCnt=markovBlnktVariables.size()/3;
		}
		else // classification mode -- sqrt(nvariables)
		{	
			randvarCnt=sqrt(markovBlnktVariables.size());
		}	
...
/*
* Predicts average value over trees for regression
* Predicts majority value over all trees for classification
*/
double 
Potential::predictSample(EMAP* evidMap, int vId)
{
	if (regressionMode)
	{
		return predictSampleRegression(evidMap, vId);
	} 
	else 
	{
		return predictSampleMajority(evidMap, vId);
	}
}

/*
* Predicts regression mode -- average value given by each tree.
*/
double 
Potential::predictSampleRegression(EMAP* evidMap, int vId)
{
	double predValSum=0;
	for(int i=0;i<dtreeSet.size();i++)
	{
...
/*
* Classification mode prediction -- majority value.
* Not sure if we will ever use this or would prefer to get distribution.
*/
double 
Potential::predictSampleMajority(EMAP* evidMap, int vId)
{
	map<double,int> predicted;
	...
	// get max over predicted values
	double predMax;
	int bestCount=0;
	for(map<double,int>::iterator it=predicted.begin(); it!=predicted.end(); it++)
	{
...
/*
*  Gets output distribution over all trees.
*/
int
Potential::predictOutputDistribution(EMAP* evidMap, int vId, map<double,double>& outcomes)
{
	if (regressionMode)
	{
		cerr << "Potential::getOutputDistribution() is not implemented for regression mode." << endl;
		return -1;
	}
	...
	for(int i=0;i<dtreeSet.size();i++)
	{
		...
		// what are possible outcomes for this leaf?
		vector<double> leafOutcomes;
		currNode->getOutputValues(leafOutcomes);
		
		// for each outcome, we add its probability
		//cout << "Tree " << i << endl;
		for (int j=0; j < leafOutcomes.size();j++)
		{
			double predVal=leafOutcomes[j];
			double prob=currNode->getOutputProbability(predVal);
			outcomes[predVal]+=prob;
			//cout << "\t" << predVal << "\t" << prob << endl;
		}
	}
	
	// Get average to create distribution.
	// is this legit? can I modify from within the map? I think so...
	double treeWeight=(1.0/dtreeSet.size());
	for (map<double,double>::iterator it=outcomes.begin(); it!=outcomes.end(); it++)
	{
		double out=it->first;
		double val=it->second;
		outcomes[out]=val*treeWeight;
		//cout << "CONSENSUS\t"<< out << "\t" << outcomes[out] << endl;
	}
...
Potential::getMSE()
{
	...
	cout << "MSE " << overallMSE1<< " MSE2 " << err2 << endl;
...
* assumes same variable structure as training data
* Writes to an output file
* New with simple version -- uses evidence sample names!
* DC Added
*/
double
...
{	
	...
	for(int i=0;i<evidCnt;i++)
	{
		...
		double diff=predval-trueval;
		double e=diff*diff;
		oFile << testEvMgr->getSampleName(i) << "\t" << trueval << "\t" << predval << "\t" << e <<  endl;
		mse=mse+e;
	}
	mse=mse/((double) evidCnt);
	if (regressionMode)
...
/*
* Assess classification error on an external test data set.
* Assumes same variable structure (row names) as training data
* Writes to an output file
* Writes one column per possible output.
* If oFile is null, only writes summary to stdout.
* DC Added
*/
double
Potential::getTestClassError(EvidenceManager* testEvMgr, ostream& oFile)
{	
	int evidCnt=testEvMgr->getNumberOfEvidences();

	vector<double> truevals; // true value
	vector<double> predvals; // majority prediction
	vector<map<double,double>> distros; // distribution of outcomes per example
	
	// what are the possible outcomes? 
	map<double,int> allOutcomes;
	...
	// for each class, get total loss
	map<double,double> classloss;
	...
	for(int i=0;i<evidCnt;i++)
	{
		...
		map<double,double> distro;
		predictOutputDistribution(evidMap, sId, distro);
		
		// store possible outcomes
		for(map<double,double>::iterator it=distro.begin(); it!=distro.end(); it++)
		{
			allOutcomes[it->first]++;
		}
		
		// majority value
		double majorityVal=predictSample(evidMap,sId);
		
		truevals.push_back(trueval);
		predvals.push_back(majorityVal);
		distros.push_back(distro);		
		
	}
	
	// print to file
	oFile << "Column\tTrueValue"; // \tMajorityValue";	
	// print each outcome as a header
	for(map<double,int>::iterator it=allOutcomes.begin(); it!=allOutcomes.end(); it++)
	{
		oFile << "\tConfIn_" << it->first << "\tIsClass_" << it->first; // << "\tLoss_" << it->first;
	}
	oFile << endl;
	
	
	// for each training example, for each class
	// print out probability of example being given that class
	for(int i=0;i<evidCnt;i++)
	{
		double trueval=truevals[i];
		double majval=predvals[i];
		map<double,double> distro=distros[i];
		oFile << testEvMgr->getSampleName(i) <<  "\t" << trueval; // << "\t" << majval;
		
		
		for(map<double,int>::iterator it=allOutcomes.begin(); it!=allOutcomes.end(); it++)
		{
			double prob=distro[it->first];
			int isClass=0;
			// loss: if we were trying to predict this value, how bad
			// would we have done?
			//double loss=0;
			if (trueval == it->first) // if this outcome is correct
			{
				isClass=1;
				//loss=1-prob;
			}
			else // if this outcome is not correct
			{
				//loss=prob;
			}
			//classloss[it->first]+=loss;
			
			
			oFile << "\t" << prob << "\t" << isClass; // << "\t" << loss;
	...
	// print out class losses
	/*for(map<double,double>::iterator it=classloss.begin(); it!=classloss.end(); it++)
	{
		double avgloss=it->second/evidCnt;
		cout << "AvgLoss\tClass_" << it->first << "\t" << avgloss << endl;		
	}*/
	truevals.clear();
	predvals.clear();
	distros.clear();
...
/**
* Dumps out target class column.
* Leaves are separated by horizontal Dummy columns. (-100)
* TODO TEST
*/
int
Potential::dumpExpressionTab(RegressionTree* theTree, int sId, ostream& oFile)
{
	vector<RegressionTree*> leaves;
	theTree->getLeafNodes(leaves);
	...
	// print header line before first example
	// need to get the variable name to do this
	bool first=true;
	...
	for(int i=0; i<evidVect.size(); i++)
	{
		...
		if (eId>=0)
		{
			...
			{
				// file header
				oFile << "Sample\t" << var->getName() << endl;
				first=false;
			}
			
			oFile << evMgr->getSampleName(eId) << "\t" << dataval << endl;	
		}
		else 
		{
			oFile << "Dummy_" << dummyCt++ << "\t" << dataval << endl;
		}	
	}
	return 0;
}
...
/**
* Dumps out tree to Cytoscape-friendly network file.
* Based on _Simple version of evidence manager -- assumes we read
* the sample names with the data.
* Each split in the tree becomes 2 edges. We annotate each edge with split value.
* Leaves will have many edges off them (one per target gene).
*
* Edge was:   Target GATA3 <=0 CEBPB
...
*/
int
Potential::dumpTreeToNetwork(RegressionTree* currNode, ostream& oFile, const char* targetName, map<int,int> &timesSeen, int sId)
{
	// if no parents, print out the header row
	if (currNode->getParent() == NULL)
	{
	...
	for(map<int,RegressionTree*>::iterator cIter=children.begin();cIter!=children.end();cIter++)
	{
		...
		else
		{
			...
			for(int i=0; i<evidVect.size(); i++)
			{
				...
				if (eId >= 0)
				{
					...
					// print edge with gene value
					oFile << testVarName << leafType << "\tterminal\t" << evMgr->getSampleName(eId) << "\tterminal\t" << targetName << "\t" << 1 << endl;
				}
				else
				{
					cout << "problem in potential:: dump network ??????" << endl;
		...
		if(childNode->getTestVariable()!=-1)
		{
			dumpTreeToNetwork(childNode, oFile,targetName, timesSeen, sId);
		}
	}
