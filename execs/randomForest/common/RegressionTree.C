...
/*
* Set regression (True) or classification mode (False)
*/
int
RegressionTree::setRegressionMode(bool doRegression)
{
...
/**
* Increment a count for seeing this output value.
*/
int 
RegressionTree::addOutputValue(double value)
{
	outputValues[value]++;
}
...
/**
* Retrieve output probability for some value.
* Return 0 if not in map.
...
/*
* Gets possible outcomes at this leaf.
*/
int
RegressionTree::getOutputValues(vector<double>& outcomes)
{
	for(map<double,int>::iterator it=outputValues.begin(); it!=outputValues.end(); it++)
	{
		// store key 
		outcomes.push_back(it->first);
	}
	return 0;
}

/*
* Create children for this node based on a split.
* Updated for classification -- hold onto distribution of outcomes in the child.
*/
int
RegressionTree::split(double atestValue, PARTITION* p)
{
	...
	for(int i=0;i<varValues.size();i++)
	{
		...
		childNode->setEvidenceManager(evMgr);
		
		// DC added 
		childNode->setRegressionMode(regressionMode);
		
		children[varValues[i]]=childNode;
		
		// loop over data points -- setDataID will push onto the vector.
		INTVECT* dataSubset=(*p)[varValues[i]];
		//cout << i << " " << varValues[i] << endl;
		
		for(int j=0; j<dataSubset->size(); j++)
		{
			childNode->setDataID((*dataSubset)[j]);
		
				
			// update the node's data distribution only if in classification mode
			// printing is useful for debug in both cases
		
			/*
			int dataID=(*dataSubset)[j];
			EMAP* evMap=evMgr->getEvidenceAt(dataID);
			Evidence* evid=(*evMap)[classVarID];
			double attrVal=evid->getEvidVal(); 
			cout << "dataID " << dataID << ", attrVal " << attrVal << endl;
			*/
				
				
			if (!regressionMode)
			{
				// we are keeping track of the distribution of output values
				// at this node
				int dataID=(*dataSubset)[j];
				EMAP* evMap=evMgr->getEvidenceAt(dataID);
				Evidence* evid=(*evMap)[classVarID];
				double attrVal=evid->getEvidVal();
				childNode->addOutputValue(attrVal);
			}
		}
...
/*
* Learn the regression tree
* TODO: make another version that does classification.
*/
int
RegressionTree::learn(double lambda, int minLeafSize,int varsToSample)
{
	if (regressionMode)
	{
		return learnRegression(lambda, minLeafSize, varsToSample);
	} 
	else
	{
		return learnClassification(lambda, minLeafSize, varsToSample);	
	}
}

/*
* Learn in regression mode (original)
*/
int
RegressionTree::learnRegression(double lambda, int minLeafSize,int varsToSample)
{
	estimateMarginal();
	...
	while(!currentLeafNodes.empty())
	{
		...
		// TODO this is where the node mean value gets set 
...
/*
* Learn in classification mode 
*/
int
RegressionTree::learnClassification(double lambda, int minLeafSize,int varsToSample)
{
	estimateMarginal();
	//This is where we do the learning loop from populateMe in the Potential class
	RegressionTree* currNode=this;
	...
	while(!currentLeafNodes.empty())
	{
		...
		for(INTINTMAP_ITER mbIter=varsTobesampled.begin();mbIter!=varsTobesampled.end();mbIter++)
		{
			...
			// will return nonzero if split not possible
			int canSplit=getPartitions_Discrete(mbIter->first,splitValue,currNode->getMarginalEntropy(),currSubset,gain);
		...
		// make sure mode is propagated
		// make sure class distributions get calculated
		currNode->split(testValue,allPartitions[testVarID]);
		// set distributions with the evMgr!!
		
		// TODO this is where the node mean value gets set -- update this one
		// where do allMeans, allVariances come from? get set during getPartitons_Cached.
		// We are repurposing mean with majority class, variance with entropy.
		// mean is sometimes not getting set correctly -- maybe if only one item?
		currNode->setChildParams(allMeans[testVarID],allVariances[testVarID],allMarginalEntropy[testVarID]);
...
/*
* Gets partitions for assumed-discrete data
*/
int 
RegressionTree::getPartitions_Discrete(int vId, double& splitValue,double marginalEntropy, vector<int>& dataSet, double& infoGain)
{
	/*
	* Here we want to partition dataSet into smaller partitions corresponding to the different values of vId. 
	* We also want to compute the entropy of the class variable using the partitions. 
	* So we store for each value of variable vId, the distribution of the class variable.
	*/
	vector<int> sortedInd;
	vector<double> sortedValues;
	vector<int> sortedInd2;
	...
	// Now we have the feature values sorted.
	int partId=1;
	...
	// left set of items is to the left of the candidate split;
	// right set of items is to the right.
	// "middle" set of items is transiently being moved from right to left.
	map<double,int> leftDistro;
	map<double,int> rightDistro;

	// -2 because we do not want anything on the right with no values.
	while(partId<sortedValues.size()-2)
	{		
		...
		// we only want to consider unique values on predictors
		// move left ID over until we get to the next value
		int leftId=partId+1;
		while(leftId<sortedValues.size() && sortedValues[leftId]==attrVal)
		{
			// if we already have created the left and right distros
			if(!first) 
			{
				int dId=sortedInd[leftId];
				EMAP* evMap=evMgr->getEvidenceAt(dId);
				...
				// move this item from the right to the left
				rightDistro[classVarVal]--;
				leftDistro[classVarVal]++;
				//cout << "Moving attr=" << attrVal << ", class=" << classVarVal << " from right to left" << endl; 
			}
			leftId++;
		}
		partId=leftId-1;
		...
		// calculate the left-hand entropy
		double classEntrLeft=0;
		double meanLeft=0;
		double varianceLeft=0;
		
		getSubEntropyDiscrete(sortedInd,0,partId,meanLeft,varianceLeft,classEntrLeft);	
		
		// calculate the right-hand entropy
		double classEntrRight=0;
		double meanRight=0;
		double varianceRight=0;
		double elemCnt=(double) (sortedValues.size()-partId-1);
		...
		// only do this if we have some items on both sides
		if(frac1>0 && frac2>0)
		{
			weightedEntropy=(frac1*classEntrLeft)+ (frac2*classEntrRight);
			double currInfoGain=marginalEntropy-weightedEntropy;
			...
			//cout << "Left " << classEntrLeft << ", Right " << classEntrRight << " InfoGain " << currInfoGain << endl;
		...
		{
			//cout << "Not enough items on each side (getPartitions_Discrete)" << endl;
	...
	// split up data points
	// storing the distribution will happen when split is formally made
	// in split()
	for(int i=0;i<sortedInd.size();i++)
	{
		int pseudoVal=-1;
...
RegressionTree::getDataAtLeaf(RegressionTree* currNode, vector<int>* evidVect)
{
	...
	cerr << "NOT IMPLEMENTED? RegressionTree::getDataAtLeaf" << endl;
	return 0;
}
...
/*
* Gets entropy/variance from a subset of values
*/
int 
RegressionTree::getSubEntropy(vector<int>& sortedInd, int start, int end,double& mean,double& variance,double& marginalEntropy)
{
	if (regressionMode)
	{
		return getSubEntropyContinuous(sortedInd, start, end, mean, variance, marginalEntropy);
	}
	else
	{
		return getSubEntropyDiscrete(sortedInd, start, end, mean, variance, marginalEntropy);
	}
}

/*
* Original -- entropy of continuous data points is calculcated as variance.
*/
int 
RegressionTree::getSubEntropyContinuous(vector<int>& sortedInd, int start, int end,double& mean,double& variance,double& marginalEntropy)
{
	marginalEntropy=0;
	mean=0;
...
/*
* Currently, variance==marginalEntropy
* Don't need a copy of this for the discrete version?
*/
int 
...
* Calculates shannon entropy OR gini
*/ 
int 
RegressionTree::getSubEntropyDiscrete(vector<int>& sortedInd, int start, int end,double& mean,double& variance,double& marginalEntropy)
{
	// count up number of each class
	map<double, int> distro;
	...
	for(int i=start;i<=end;i++)
	{
		...
		// get data point here
		double classVarVal=(*evMap)[classVarID]->getEvidVal();
		map<double,int>::iterator it=distro.find(classVarVal);
		if (it == distro.end())
		{
			distro[classVarVal]=0;
		}
		distro[classVarVal]++;
	}
	
	int totItems=(end-start)+1;
	// calculate entropy or gini
	// mean will be majority class value
	marginalEntropy=0;
	mean=0;
	// TODO add gini option later
	getShannonEntropy(distro, totItems, mean, marginalEntropy);
	//cout << marginalEntropy << endl;
	
	// just duplicate variance as entropy
	variance=marginalEntropy;
	
	return 0;
}

/**
* Calculates Shannon entropy over the outcomes in the subset
* Returns majority outcome and entropy
*/
int RegressionTree::getShannonEntropy(map<double,int>& distro, int numItems, double& majority, double& marginalEntropy)
{
	// mean will be the majority class
	double maxFrac=0;
	
	for (map<double,int>::iterator it=distro.begin(); it!=distro.end(); ++it)
	{
		// fraction of items in this class
		double frac=it->second / (numItems*1.0);
		marginalEntropy+=frac*log2(frac);
		
		// save majority class as "mean"
		if (frac>maxFrac)
		{
			maxFrac=frac;
			majority=it->first;
	...
	// negative for entropy
	marginalEntropy=marginalEntropy*(-1.0);
	return 0;
}

/**
* Calculates gini impurity over a set of outcomes (assumed discrete)
*
*/
int RegressionTree::getGiniImpurity(map<double,int>& distro, int numItems, double& majority, double& impurity)
{
	// mean will be the majority class
	double maxFrac=0;
